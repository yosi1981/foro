...
class Timezone {
     ...
     * @return string
     */
    public function dropdownMenu($selected = null)
    {
        $options = '<select class="form-control" name="timezone" id="timezone">';
        if (!$selected) {
            $options .= '<option selected="true" disabled="disabled"> ' . trans('site.select_one') . '</option>';
        }
        foreach ($this->timezoneList as $key => $timezone) {
            $selectedText = $selected == $timezone ? 'selected="selected"' : '';
            $options .= "<option {$selectedText} value='{$timezone}'>{$key}</option>";
        }
        $options .= '</select>';
        return $options;
    }
